...
<script>
  ...
  badge.addEventListener("click", (event) => {
    ...
    // Calculate the click position relative to the badge
    const clickX = event.clientX - rect.left;
    // Define the action for the left side (logo)
    if (clickX < rect.width / 2) {
      window.open("https://www.linkedin.com/feed/", "_blank"); // Open LinkedIn's company page
    }
    // Define the action for the right side (text)
    else {
      window.open("https://www.linkedin.com/in/patryk-skarżyński-b20690173/", "_blank"); // Open your LinkedIn profile
    }
  });
</script>
</body>
